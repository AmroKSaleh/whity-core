...
/**
 * GrantDesktopPluginsReadToAdmin migration (WC-desktop-plugins).
 *
 * Upserts the `desktop-plugins:read` permission into the catalogue and grants
 * it to the built-in `admin` role, so a fresh database can list/download
 * desktop plugin releases out of the box.
 *
 * Mirrors the catalogue-upsert + grant pattern of migrations 079/086.
 * down() reverses only what up() added.
 */
class GrantDesktopPluginsReadToAdmin
{
    /**
     * @var array<string, string>
     */
    private const PERMISSIONS = [
        CorePermissions::DESKTOP_PLUGINS_READ => 'List and download desktop plugin releases (global catalog)',
    ];
