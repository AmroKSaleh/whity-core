...
/**
 * GrantAuthProvidersManageToAdmin migration (WC-e6287).
 *
 * Registers the `auth_providers:manage` permission in the catalogue and grants
 * it to the seeded `admin` role, so a fresh database can manage its tenant's
 * identity-provider (SSO/OIDC) configurations out of the box. Tenant-scoped: the
 * IdentityProvidersApiHandler resolves TenantContext, so an admin manages only
 * their own tenant's providers.
 *
 * Mirrors the established catalogue-upsert + grant pattern (migrations 043/026/016),
 * resolving ids defensively (skip on a partially-seeded database). down() reverses
 * only what up() added.
 */
class GrantAuthProvidersManageToAdmin
{
