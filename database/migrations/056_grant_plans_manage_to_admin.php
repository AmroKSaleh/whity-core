...
/**
 * GrantPlansManageToAdmin migration (WC-plans).
 *
 * Registers the `plans:manage` permission and grants it to the seeded `admin`
 * role. Like `entitlements:manage`, this is a PLATFORM capability: the
 * PlansApiHandler additionally requires the caller to be acting in the system
 * tenant (id 0), so granting it to the global admin role is safe — a regular
 * tenant admin holds the permission but the system-tenant gate blocks them.
 *
 * Mirrors the established catalogue-upsert + grant pattern (migrations 052/054).
 * down() reverses only what up() added.
 */
class GrantPlansManageToAdmin
{
