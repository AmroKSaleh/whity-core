...
/**
 * GrantDesktopAppUpdatesReadToAdmin migration (WC-app-self-update).
 *
 * Upserts the `desktop-app-updates:read` permission into the catalogue and
 * grants it to the built-in `admin` role, so a fresh database can serve the
 * desktop app's self-update manifest out of the box.
 *
 * Mirrors the catalogue-upsert + grant pattern of migration 098
 * (GrantDesktopPluginsReadToAdmin). down() reverses only what up() added.
 */
class GrantDesktopAppUpdatesReadToAdmin
{
    /**
     * @var array<string, string>
     */
    private const PERMISSIONS = [
        CorePermissions::DESKTOP_APP_UPDATES_READ => 'Check for and read the desktop app self-update manifest',
    ];
