...
/**
 * GrantPasswordResetsApproveToAdmin migration (WC-password-reset-2fa-recovery).
 *
 * Registers the `password_resets:approve` permission in the `permissions`
 * catalogue and grants it to the seeded `admin` role, so a fresh database can
 * review pending self-service password-reset requests out of the box.
 *
 * Unlike `registrations:approve` (migration 043), this permission is NOT
 * system-tenant-restricted — the requesting user's account and tenant already
 * exist, so their OWN tenant's admin reviews it (the handler scopes the queue
 * to tenants where the requester holds an active membership).
 *
 ...
 */
class GrantPasswordResetsApproveToAdmin
{
    /**
