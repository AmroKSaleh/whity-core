...
/**
 * Static-analysis guard that enforces the platform's #1 isolation invariant
 * (WC-192): every SELECT/UPDATE/DELETE on a TENANT-OWNED table must bind a
 * `tenant_id` predicate, unless the table is a sanctioned global one or the
 * statement carries an explicit, reasoned ignore annotation.
 *
 * The guard parses PHP source with the native tokenizer (no new dependencies),
 * stitches each SQL string literal together with the literals that build the
 * SAME statement (`.` concatenation and `$sql .= '...'` / `$x = '...' . $where`
 * builder fragments within the enclosing function), and decides per statement:
 *
 ...
 * does NOT scope the row — the guard requires `tenant_id` in a comparison
 * position (after WHERE/AND/ON or immediately before =, IN, IS, <, >, !=, <>).
 *
 * This is a heuristic SQL guard, deliberately conservative: it errs toward
...
final class TenantPredicateGuard
{
    ...
    /** SQL DML verbs the guard polices. INSERT is out of scope (it sets, not selects, a row). */
    ...
    private function extractStatements(array $tokens): array
    {
        ...
        $flushBuilders = static function () use (&$builderVars, &$statements): void {
            ...
            // still judged, so a deferred fragment can't escape the guard.
    ...
    /**
     * Whether the SQL is a SELECT/UPDATE/DELETE the guard polices.
     *
     ...
     * the guard on the trailing `DO UPDATE SET`, so they are excluded up front.
     */
    ...
    private function tenantOwnedTablesReferenced(string $sql): array
    {
        ...
        foreach ($matches[1] as $candidate) {
            ...
            if (SanctionedGlobalTables::isGlobal($table)) {
                continue; // Allowlisted global table — never flag.
            }
            if (TenantOwnedTables::isTenantOwned($table)) {
                $found[$table] = true;
            }
        }
