...
import { SSO_ERROR_KEYS } from './sso-error-keys';
...
/**
 * Which translation keys `ssoErrorMessage` below can actually produce.
 *
 * It reaches `t()` through a variable (`t(entry.key, entry.fallback)`), which
 * no static scanner can read — so the reachable set is declared here and the
 * extractor takes the catalogue from this block rather than pretending the
 * scan saw them. The declaration is what a translator gets, so the two must
 * not drift; `web/__tests__/login-sso-key-declaration.test.ts` fails if they
 * do.
 *
 * Declared HERE and not beside the map in `./sso-error-keys`: the extractor
 * wants the declaration in the same file as the dynamic call, and it is right
 * to — it is a claim about this call site, not about that table.
 *
 * @i18n-keys auth
