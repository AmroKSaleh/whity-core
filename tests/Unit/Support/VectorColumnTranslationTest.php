...
final class VectorColumnTranslationTest extends TestCase
{
    ...
    /**
     * ONE test, TWO meanings, because the engines genuinely differ and hiding
     * that behind a skip would leave the more important half unrun.
     *
     * On SQLite the point is the translation: `vector(n)` becomes TEXT and the
     * literal round-trips, so an adopter's migration does not abort the schema
     * build.
     *
     * On PostgreSQL the point is that the IMAGE actually ships pgvector — the
     * whole reason the base image changed. A real `vector(384)` column is
     * created and read back. If this fails on the Postgres path, the compose
     * and CI image pins are wrong, which is exactly what it should catch.
     */
    public function testAVectorColumnWorksOnBothEngines(): void
    {
        $pdo = $this->sqlite();

        if ($this->isPostgres($pdo)) {
            $pdo->exec('CREATE EXTENSION IF NOT EXISTS vector');
            $pdo->exec('CREATE TABLE embeddings (id INTEGER PRIMARY KEY, embedding vector(3))');
            $pdo->exec("INSERT INTO embeddings (id, embedding) VALUES (1, '[1,2,3]')");

            // The distance operator, which is the capability the extension is
            // here for and the one SQLite cannot emulate.
            $distance = $this->queryOne($pdo, "SELECT embedding <=> '[1,2,3]' FROM embeddings WHERE id = 1")
                ->fetchColumn();
            self::assertSame(0.0, (float) $distance, 'a vector is at zero cosine distance from itself');
        ...
        self::assertSame(
            '[1,2,3]',
            $this->queryOne($pdo, 'SELECT embedding FROM embeddings WHERE id = 1')->fetchColumn()
        );
    }
    ...
    public function testCreateExtensionIsANoOpRatherThanASyntaxError(): void
    {
        ...
        // A plugin's migration opens with this. On SQLite it was a hard syntax
        // error before the translation rule, aborting the entire schema build
        // rather than the one statement that could not apply. On PostgreSQL it
        // is the real thing, and succeeding proves the image ships pgvector.
        $pdo->exec('CREATE EXTENSION IF NOT EXISTS vector');
