...
One lookup inherently runs before any tenant context exists: the login path resolves an account by email (`AuthHandler`). Two separate things make that safe, and they are worth keeping apart.

**It is unambiguous — this was issue #181, and it is resolved.** The old schema's `UNIQUE(tenant_id, email)` allowed the same address in two tenants, so a login could not say which was meant. The identity cutover closed it: `profile_emails` holds addresses under a **global** `UNIQUE(email)` (migration `029`), a profile is one person across the whole deployment, and which tenant they act in is a separate `memberships` row resolved into `active_tenant_id`. An email now identifies exactly one profile, so the pre-context lookup has a single answer by construction rather than by convention. See [ADR 0005](../adr/0005-identity-tenant-membership-model.md) for the model; migration `047` cites #181 as the resolved case its own namespacing mirrors.

**It is verified — an unverified address never authenticates** ([ADR 0005](../adr/0005-identity-tenant-membership-model.md) §2). Verification is a gate, not a flag: an unverified email is an unproven claim to an identity, and the entire tenant boundary hangs off which profile the caller turns out to be. The refusal is the **generic** invalid-credentials response rather than a verification-specific one, and it is latency-compensated — saying "that address exists but is unverified" is a user-enumeration oracle, and saying it *faster* than the password path is the same oracle with extra steps.
