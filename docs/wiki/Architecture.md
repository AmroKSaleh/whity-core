...
| Query-level tenant scoping | `ScopesToTenant` (trait) | `src/Core/Database/ScopesToTenant.php` |
...
2. **`ScopesToTenant` trait** (`src/Core/Database/ScopesToTenant.php`) — query-level enforcement. `tenantScopedQuery()` / `applyTenantScope()` rewrite SQL to inject a parameterised `tenant_id` predicate (SELECT/UPDATE/DELETE) or auto-populate `tenant_id` on INSERTs. Security model: system mode → unscoped; unresolved tenant (and not system mode) → **fail closed** (`TenantScopeException`); resolved tenant → predicate injected via a **bound parameter** (never string-interpolated). Statements it cannot rewrite safely (JOINs, subqueries, `INSERT ... SELECT`, columnless INSERTs) are *refused* rather than run unscoped.
